...
const Checkout = () => {
  ...
  // Verificar carrito vacío al cargar el componente. Si estamos procesando
  // un pago (isProcessing=true) no redirigimos ni mostramos el mensaje.
  useEffect(() => {
    if (isProcessing) return; // durante procesamiento no forzar redirección
  ...
  const shippingCost = (deliveryOptions.find(o => o.value === shippingInfo.deliveryOption) || { cost: 0 }).cost || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Llamar a endpoint de pago (simulado en backend) pasando total + items + shipping
      const paymentResult = await api.pago(total, cart, shippingCost, shippingInfo);

      // Si el pago fue exitoso, el backend intentará crear ventas y descontar stock.
      // Si fue fallido, el backend habrá registrado el intento y devolverá success=false.
      if (!paymentResult) {
        throw new Error('Respuesta inválida del backend');
      }

      if (paymentResult.success === false) {
        // navegar a pantalla de error mostrando el detalle del intento
        navigate('/checkout/error', { state: { paymentResult } });
        return;
      }

      // Limpiar solo las unidades que el backend confirmó como vendidas
      if (paymentResult && Array.isArray(paymentResult.items)) {
        // items array contiene productoId que coincide con el id del carrito (si se envió así)
        for (const resItem of paymentResult.items) {
          if (resItem.status === 'sold') {
            // eliminar la cantidad vendida de ese producto
            eliminarDelCarrito(resItem.productoId, resItem.cantidad);
          }
        }
      } else {
        // fallback: si no hay detalle, limpiar todo
        cart.forEach(item => {
        ...
        // Los componentes escuchan estos eventos y volverán a pedir datos al backend.

      // Notificar a otras partes de la app que las órdenes y productos cambiaron
      window.dispatchEvent(new CustomEvent('ordersUpdated'));
      window.dispatchEvent(new CustomEvent('productsUpdated'));

      // Redirigir a página de éxito y pasar resultado para mostrar resumen
      // Incluir paymentId en la URL para asegurar que la ruta coincida
      const paymentId = paymentResult.paymentId || paymentResult.payment_id || '';
      navigate(`/checkout/success/${paymentId}`, {
        state: {
          paymentResult,
          shippingInfo,
          subtotal,
  ...
  // Evitar renderizar el mensaje de "carrito vacío" mientras se procesa el pago.
