...
const Ofertas = () => {
  ...
  useEffect(() => {
    const loadOfertas = async () => {
      ...
      // Obtener productos completos para las ofertas (preferir backend)
      let allProducts = [];
      try {
        ...
        console.warn('No se pudo obtener productos desde backend en Ofertas, usando dataStore', err && err.message);
      ...
      const productosConOferta = ofertasAdmin.map(oferta => {
        const producto = allProducts.find(p => String(p.id) === String(oferta.productId));
      ...
      const blacklistNames = new Set(['producto prueba', 'prueba producto', 'prueba', 'producto de prueba', 'teclado mecanico rgb']);
      const ofertasFiltradas = productosConOferta.filter(p => {
        const name = (p.name || p.nombre || '').toString().toLowerCase().trim();
        const category = (p.category || p.categoria || '').toString().toLowerCase();
        if (blacklistNames.has(name)) return false;
        if (category && category.includes('perif')) return false;
        return true;
      });
