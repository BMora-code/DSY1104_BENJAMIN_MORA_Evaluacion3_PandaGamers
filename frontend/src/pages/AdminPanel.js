...
const AdminPanel = () => {
  ...
  const loadData = async () => {
    // Cargar productos desde backend y añadir productos administrados localmente
    try {
      const srvProds = await api.getAllProducts();
      const mapped = (Array.isArray(srvProds) ? srvProds : []).map(p => ({
        id: p._id || p.id,
      ...
      // Añadir productos creados por admin en localStore
      const adminProducts = localStore.getAdminProducts();
      setProducts([...mapped, ...adminProducts]);
    } catch (err) {
      // Si falla el backend, mostrar solo productos administrados en local
      setProducts(localStore.getAdminProducts());
    }

    // Cargar órdenes y usuarios desde endpoints debug del backend
    try {
      const [srvUsers, srvVentas] = await Promise.all([
        api.getDebugUsers().catch(() => []),
        api.getDebugVentas().catch(() => [])
      ]);

      if (Array.isArray(srvUsers)) {
        const mapped = srvUsers.map(u => ({
          id: String(u.id || u._id || u._id),
          username: u.nombre || u.username || 'Sin nombre',
          email: u.email || '',
          password: '*******',
          role: u.role || 'user',
          date: u.createdAt || u.created_at || null
        }));
        setUsers(mapped);
      }

      if (Array.isArray(srvVentas)) {
        const mappedOrders = srvVentas.map(v => ({
          id: String(v._id || v.id),
          userId: v.comprador || v.compradorId || (v.user && (v.user.email || v.user.nombre)) || 'desconocido',
          total: v.total || 0,
    ...
    // Cargar ofertas desde localStore
    setOfertas(localStore.getOfertas());
  };

  const handleUpdateStock = async (product, newStock) => {
    // actualizar en backend si el producto existe allí (buscar por nombre)
    try {
      const srv = await api.getProductByName(product.name);
      if (srv && srv._id) {
        await api.updateProductStock(srv._id, newStock);
      }
    } catch (err) {
      // ignore backend errors; seguir con la actualización local
      console.warn('No se pudo actualizar stock en backend para', product.name, err && err.message);
    }
    // actualizar estado local (si es producto admin lo persistimos en localStore)
    if (String(product.id).startsWith('100')) {
      try { localStore.updateAdminProduct(product.id, { stock: newStock }); } catch (e) {}
    }
    setProducts(prevProducts => prevProducts.map(p => p.id === product.id ? { ...p, stock: newStock } : p));
  ...
  const handleProductSubmit = (e) => {
    ...
    if (editingProduct) {
      // Si es producto administrado localmente, persistir cambio, si no mostrar aviso
      if (String(editingProduct.id).startsWith('100')) {
        const updated = localStore.updateAdminProduct(editingProduct.id, productData);
        setProducts(prevProducts => prevProducts.map(p => p.id === editingProduct.id ? { ...p, ...productData } : p));
      } else {
        // No existe endpoint para editar productos del backend; crear override local
        const override = localStore.createAdminProduct({ ...productData, name: productData.name });
        setProducts(prev => prev.map(p => p.id === editingProduct.id ? { ...p, overrideId: override.id } : p));
      }
    } else {
      const newProduct = localStore.createAdminProduct(productData);
  ...
  const handleUserSubmit = (e) => {
    ...
    (async () => {
      try {
        const payload = { nombre: userForm.username, email: userForm.email, password: userForm.password, role: userForm.role };
        await api.register(payload);
        await loadData();
        resetUserForm();
      } catch (err) {
        alert('No se pudo crear el usuario: ' + (err?.response?.data?.message || err.message || String(err)));
      }
    })();
  };
  ...
  const handleDeleteProduct = (id) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar este producto?")) {
      // Solo soportamos eliminar productos creados desde el panel (admin local)
      if (String(id).startsWith('100')) {
        localStore.deleteAdminProduct(id);
        setProducts(prev => prev.filter(p => p.id !== id));
        window.dispatchEvent(new CustomEvent('productsUpdated'));
      } else {
        alert('No es posible eliminar productos que provienen del backend.');
      }
    }
  };
  ...
  const handleDeleteUser = (id) => {
    ...
    if (window.confirm("¿Estás seguro de que quieres eliminar este usuario?")) {
      alert('Eliminar usuarios debe realizarse desde el backend. Esta acción no está disponible en el panel local.');
    }
  };

  const handleDeleteOrder = (id) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar esta orden?")) {
      // No se permite eliminar órdenes del backend desde el frontend.
      alert('Eliminar órdenes debe hacerse desde el backend o administración del servidor.');
    }
  };
  ...
  const handleOfertaSubmit = (e) => {
    ...
    if (editingOferta) {
      localStore.updateOferta(editingOferta.id, ofertaData);
    } else {
      localStore.createOferta(ofertaData);
    }
                      ...
                      <button
                        ...
                        onClick={async (e) => {
                          ...
                          if (newStock !== product.stock) {
                            await handleUpdateStock(product, newStock);
                          }
                        }}
                      >
                        💾 Actualizar
            ...
            <div className="card mb-4" style={{ background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)' }}>
              <div className="card-body">
                ...
                <form onSubmit={(e) => {
                  ...
                  // Crear administrador via API
                  (async () => {
                    try {
                      const payload = { nombre: userForm.username, email: userForm.email, password: userForm.password, role: 'admin' };
                      await api.register(payload);
                      await loadData();
                      resetUserForm();
                    } catch (err) {
                      alert('No se pudo crear el administrador: ' + (err?.response?.data?.message || err.message || String(err)));
                    }
                  })();
                }}>
                  <div className="row">
