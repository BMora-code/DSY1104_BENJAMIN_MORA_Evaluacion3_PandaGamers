          ...
          <div className="col-lg-6">
            ...
            <div className="text-center mb-5">
              ...
              {paymentResult?.message && (
                <div className="alert alert-warning mt-3">{paymentResult.message}</div>
              )}
            </div>

            {/* Posibles razones del error */}
              {paymentResult && Array.isArray(paymentResult.items) && (
                <div className="card mb-4">
                  <div className="card-header"><strong>Detalle del intento</strong></div>
                  <div className="card-body">
                    <ul>
                      {paymentResult.items.map((it, idx) => (
                        <li key={idx}>
                          {it.nombre || it.productoId} — <strong>{it.status}</strong>{it.reason ? `: ${it.reason}` : ''}
                        </li>
                      ))}
                    </ul>
                  </div>
